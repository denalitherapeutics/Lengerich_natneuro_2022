...
```{r}
metadata <- read.csv(here("inst", "extdata", "Study2_sample_metadata.csv"))
...
for (i in 1:length(Pool1_samples)) {
  object <- Seurat::Read10X(
    # FIXME: use file.path instead of paste0 for paths on your filesystem.
    # (For AWS paths, use paste instead, because you know the delimiter for sure
    # ).
    file.path(
...
for (i in 1:length(Pool2_samples)) {
  # FIXME: should this be your own s3.read10X function? Seurat::Read10X doesn
  # have a `region` argument.
  object <- Seurat::Read10X(
    file.path(
      "pool2", "outs", "per_sample_outs", Pool1_samples[i], "count",
      "sample_filtered_feature_bc_matrix"
    ),
    region = "us-east-1",
    min.cells = 3,
...
for (i in 1:length(Pool3_samples)) {
  # FIXME: should this be s3.read10X ?
  object <- Seurat::Read10X(
    paste0(
